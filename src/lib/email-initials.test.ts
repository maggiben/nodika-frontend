...
describe("emailInitials", () => {
  test("uses the first two letters of the local part in order", () => {
    expect(emailInitials("maria@example.com")).toBe("MA");
    expect(emailInitials("ben.maggi@nodika.dev")).toBe("BE");
    expect(emailInitials("maggi.ben@nodika.dev")).toBe("MA");
    expect(emailInitials("  BMaggi@company.com ")).toBe("BM");
  });
