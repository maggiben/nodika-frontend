...
The application SHALL use `/` as a project-status landing page that visualizes the selected locally stored snapshot JSON with progress labels, MUI X Charts visualizations, and MUI X Community Data Grid task tables. Objective and context task Data Grids SHALL support client-side column sorting and filtering. When live parsed progress is available for the selected project, overall and matching task stats SHALL present baseline (snapshot), current (parsed), and delta values together.
...
#### Scenario: Comparing baseline and live progress

- **WHEN** a user opens `/` with a selected snapshot
- **AND** live parsed progress is available for that project
- **THEN** the overall stats area SHALL show baseline overall percent, current overall percent, and their delta
- **AND** objective tasks with matching live reports SHALL show baseline, current, and delta progress
