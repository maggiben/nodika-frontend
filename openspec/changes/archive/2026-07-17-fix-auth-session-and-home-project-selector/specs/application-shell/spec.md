...
The application SHALL render a shared navbar from the locale layout on every localized route that is allowed to render, including a Nodika brand mark and session-aware controls. The navbar SHALL show a project selector and obra progress chip only on the localized home route (`/{locale}` or `/{locale}/`) when projects exist / progress is available as already specified. When authenticated, the avatar menu SHALL include an Upload snapshot action that navigates to the localized `/upload` route, theme preferences where applicable, and logout. Unauthenticated users SHALL be redirected to login before app routes render, so signed-out browsing of protected routes SHALL not occur.
...
#### Scenario: Switching projects from the navbar on Home

- **WHEN** an authenticated user is on `/{locale}` or `/{locale}/`
- **AND** the project library contains one or more projects
...
#### Scenario: Hiding project chrome off Home

- **WHEN** an authenticated user is on a non-home localized route such as `/{locale}/settings`, `/{locale}/staff`, or `/{locale}/upload`
- **THEN** the navbar SHALL not show the project selector
- **AND** SHALL not show the obra progress chip
